...
alias weechat='dtach -A weechat weechat'
alias mutt='dtach -A mutt mutt -F $XDG_CONFIG_HOME/mutt/muttrc'
...
# Email
#alias mail='mbsync -c ~/.config/isync/isyncrc'
#alias mailc='vim -c ":NotMuch"'
...
#Youtube-dl + mpv goodnes
ytp () {
  url="$(xclip -o)"
  youtube-dl -F "${url}"
  read "f?selection >  "
  mpv --ytdl-format="$f" "ytdl://${url}"
}

ytp_audio () {
  mpv --ytdl-format=171 "ytdl://$(xclip -o)"
}
ytp360 () {
  mpv --ytdl-format=18 "ytdl://$(xclip -o)"
}

ytp720 () {
  mpv --ytdl-format=22 "ytdl://$(xclip -o)"
}
